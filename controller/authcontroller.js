...
exports.Sendsignupemail = catchAsync(async (req, res, next) => {
    ...
    let Randomcode = Math.trunc(Math.random() * 10000);
    req.body.confirmationcode = Randomcode
    const url = `${req.protocol}://${req.get('host')}/me`;
    await new Email(req.body.email, url, req.body.confirmationcode).sendemail();
...
exports.Signup = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(
          new AppError('please enter the code provided to your email address')
     )}else{
         user.verified = true
...
exports.Login = catchAsync(async (req, res, next) => {
  ...
  const loggedinuser = await User.findOne({ email }).select('+password -role');
  if (
    !loggedinuser ||
...
exports.protectroutes = catchAsync(async (req, res, next) => {
  ...
  if (Currentuser.changedPasswordAfter(decoded.iat)) {
    return next(
      new AppError(
        'you have recently changed the password please login again',
        401
      )
    );
  }
...
exports.restrictTo = (...roles) => {
  ...
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError(
          'you are not allowed to do this act this is restricted only for admins!!!',
          403
        )
      );
    }
...
exports.forgotpassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError('your request has been faild please try again later', 500)
    );
  }
});
...
exports.resetpassword = catchAsync(async (req, res, next) => {
  ...
  //if token expires there will be no user
  if (!user) {
    return next(
      new AppError(
        'user does not exists or the resettoken has been expired',
        400
      )
    );
  }
